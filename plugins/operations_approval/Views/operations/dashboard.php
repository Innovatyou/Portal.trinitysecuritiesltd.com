...
<?php foreach ($recent as $row) { ?><tr><td><strong><?php echo anchor(get_uri('operations/view/' . $row->id), esc($row->request_no ?: ($row->status === 'draft' ? app_lang('draft') : '—'))); ?></strong></td><td><?php echo esc($row->title); ?></td><td><?php echo esc($row->workflow_name); ?></td><td><span class="oa-status oa-status-<?php echo $statusClass($row->status); ?>"><?php echo esc(ucwords(str_replace('_', ' ', $row->status))); ?></span></td><td><?php echo format_to_datetime($row->created_at); ?></td></tr><?php } ?>
